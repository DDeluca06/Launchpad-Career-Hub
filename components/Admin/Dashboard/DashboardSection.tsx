    ...
    const fetchData = async () => {
      try {
        ...
        const updatedStats = initialStats.map(stat => {
          ...
          console.log(`Trying to map "${label}" to API keys`);
          
          // First check for exact match with the label itself (our most reliable option)
          if (data.stats[label] !== undefined) {
            console.log(`Found direct match with label: ${label} = ${data.stats[label]}`);
            return {
              ...stat,
          ...
          console.log('Available keys in API response:', Object.keys(data.stats));
          
          // Try each possible key
          for (const key of possibleKeys) {
            if (key && data.stats[key] !== undefined) {
              console.log(`Found match: ${key} = ${data.stats[key]}`);
              return {
                ...stat,
          ...
          console.log(`No matching key found for ${label}. Using default value.`);
          return stat;
        });
