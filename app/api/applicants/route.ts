...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('Using Prisma filter:', filter);
    ...
    if (sort) {
      switch (sort) {
        case 'name':
          orderBy = [
            { first_name: sortDir || 'asc' },
            { last_name: sortDir || 'asc' }
          ];
          break;
        case 'newest':
    ...
    try {
      ...
      if (dateFilter && dateFilter !== 'all') {
        ...
        console.log(`Date filter applied, now ${filteredUsers.length} users remaining`);
      }
      ...
      if (minApplications && parseInt(minApplications) > 0) {
        ...
        console.log(`Min applications filter applied, now ${filteredUsers.length} users remaining`);
      }
      ...
      console.log(`Returning ${applicants.length} applicants with stats`);
    ...
    return NextResponse.json(
      { error: 'Failed to fetch applicants', details: error.message || 'Unknown error' },
      { status: 500 }
    );
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { firstName, lastName, email, password, program } = body;
    ...
    const newUser = await prisma.users.create({
      data: {
        ...
        is_admin: false,
        is_active: true,
        program: programType as any, // Using type assertion
      },
    });
